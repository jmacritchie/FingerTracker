...
/* This set of correlation functions uses the openCV library for computer image processing*/


/* search and template function
inputs: CvPoint *pt is feature point to be tracked
	IplImage *picture is image that feature point was taken from
outputs: IplImage *corrblock needs to be 3 channel 8bit image
	IplImage *mask needs to be binary image
		these outputs feed in to correlation functions cvMatchTemplate
*/
void Correlation::search_and_template(CvPoint pt, IplImage *picture, IplImage *corrblock, IplImage *mask)
{
int x=pt.x;
int y=pt.y;
//cout << "corrblock and mask at point   "<< x<<","<<y <<"\n";
int p,q,m,n,r,w=0;
CvScalar a = cvScalar(0,0,0,0);
CvScalar c = cvScalar(0,0,0,0);
cvZero (corrblock);
cvZero (mask);
//cout << "picture "<<picture->width<<","<<picture->height;
for (p=(x-8);p<=(x+8);p++) {
	for (q=(y-8);q<=(y+8);q++) {
...
/*
masktemplate function: recalculates search area
inputs : CvPoint pt is point of feature to be tracked
	IplImage *picture is frame that contains feature
outputs: IplImage *mask is binary image containing search area
*/
...
CvPoint Correlation::max_correlation(CvPoint point,IplImage *corrblock, IplImage *maskarea, IplImage *frame)
{
	...
	int z=0;
	double maxvalue=0.0; 	//maxvalue MUST	be initialised here!!!!
...
for (p=x-8;p<x+8;p++){
	for(q=y-8;q<y+8;q++){
		...
		for (inner_length=0;inner_length<corrblock->height;inner_length++){
			...
			//if(inner
			//cout << " corr " << correlation  ;
			//corresponding value is r+w and s+l
			//fout << c1.val[0] <<"\t"<< c2.val[0] << "\t" << correlation<< "\t" << newpoint.x << "," << newpoint.y<<"\n"; 
			}
		} //cout << "\n";
	//if value of corrvalue is bigger than previous value, make new value
	//and also return corresponding values of r and s at same time
	z++;
